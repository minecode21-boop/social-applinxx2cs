...
public class SocialServer {
    ...
    public static void main(String[] args) throws Exception {
        // 1. Get Port from Render (or default to 8080 for local)
        int PORT = getPort();

        // 2. Initialize Database
        initDB();

        // 3. Start Server (Binding to 0.0.0.0 is CRITICAL for Render)
        ...
        System.out.println(">> SOCIAL SERVER (POSTGRES EDITION) STARTED ON 0.0.0.0:" + PORT);
        server.start();
    }
    ...
    static int getPort() {
        ...
        return 8080; // Default for local testing
    }

    // --- DATABASE HELPERS ---
    
    public static Connection connect() throws SQLException {
        // 1. GET DB_URL FROM ENVIRONMENT (Render sets this)
        String url = System.getenv("DB_URL");
        
        // 2. Fallback for local testing
        if (url == null || url.isEmpty()) {
             System.out.println("! No DB_URL found. Trying local postgres...");
             url = "jdbc:postgresql://localhost:5432/social_test"; 
        }
    ...
    static class StaticHandler implements HttpHandler {
        public void handle(HttpExchange t) throws IOException {
            ...
            if (!file.exists()) {
                ...
                t.sendResponseHeaders(404, r.length()); t.getResponseBody().write(r.getBytes()); t.close(); return;
            }
    ...
    static class AuthHandler implements HttpHandler {
        ...
        public void handle(HttpExchange t) throws IOException {
            ...
            if (parts.length < 2) { send(t, "Error", 400); return; }
